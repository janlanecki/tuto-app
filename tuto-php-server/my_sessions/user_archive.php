...
$queryResult = pg_query($link,
    "{$withClause}
     SELECT DISTINCT sessions.*, participants.*
     FROM sessions JOIN participants ON participants.session_id = sessions.id 
     ...
     AND sessions.due_date < now()
     ORDER BY sessions.due_date DESC");
