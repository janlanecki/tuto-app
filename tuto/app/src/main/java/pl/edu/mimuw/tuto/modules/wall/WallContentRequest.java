...
public class WallContentRequest extends JsonRequest<ArrayList<Session>> {
  private final static String URL = "https://.../session.php";
   ...
   */
  public WallContentRequest(JSONObject postParams, Listener<ArrayList<Session>> listener, ErrorListener errorListener) {
  ...
  protected Response<ArrayList<Session>> parseNetworkResponse(NetworkResponse response) {
    try {
      Log.d("WallContentResponse", new String(response.data));
